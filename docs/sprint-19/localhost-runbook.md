...
## 4) Validar comparaciones persistentes

1. iniciar sesión como padre
2. ir a `/compare/north-hills-college,colegio-san-lucas`
3. usar botón `Guardar comparación`
4. abrir `/parent-dashboard` y verificar bloque de comparaciones guardadas

## 5) Validar endpoint backend (opcional)

```bash
curl -H "x-admin-key: dev-admin-key" \
  "http://localhost:4000/v1/parents/<USER_ID>/comparisons"
```
